...
public class AlignPegZedV2 extends DriveMacro {
	...
	public AlignPegZedV2(
			DriveTrain pDriveTrain, 
			PoseEstimator pPoseEstimator, 
			XboxController pXboxController) {
...
//		};
		...
		mLRInRange = new PersistentlyInRange(mLRPID, -1, ZedAlignmentConstants.LR_INITIAL_TOLERANCE);
	...
	private SwerveMovement getMovementGoingForward()
	{
		...
		double lrDir = 90 - mPoseEstimator.getAngle();
		double fbDir = -mPoseEstimator.getAngle();
	...
	private void handleStates()
	{
		...
		case ALIGN_APPROX:
			...
			break;
		case FORWARD_MANUAL:
			if (mPoseEstimator.getFB() > ZedAlignmentConstants.RANGE_START_COUNTING)
			{
				mTimeStartedForward = System.currentTimeMillis();
			}
			
			if (System.currentTimeMillis() - mTimeStartedForward > ZedAlignmentConstants.TIME_FORWARD_MILLIS)
			{
